...
async function OpenAiKey() {
  ...
  await page.goto('https://freeopenai.xyz/');
  ...
  for (const element of elements) {
    ...
    if (text.includes('sk-')) {
      ...
      const skWords = words.filter(word => word.includes('sk-'));
      if (skWords.length > 0) {
        skContent.push({
       words: skWords
        });
  ...
  const wordsArray = skContent.map(obj => obj.words).flat();
  return wordsArray
  await browser.close();
  }