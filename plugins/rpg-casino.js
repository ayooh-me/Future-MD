...
let imgr = flaaa.getRandom()
    ...
    conn.sendButton(m.chat, caption, wm, imgr + "LOSER", [["Try Again " + args[0], "/casino " + args[0]]], m, { mentions: conn.parseMention(caption) })
    } else if (Aku < Kamu) {
    let caption = `                💰 *C A S I N O* 💰\n\n${htjava} *@${m.sender.split("@")[0]}* - [USER]\n┗┅⭑ ${Kamu} Point\n${htjava} *@${conn.user.jid.split("@")[0]}* - [BOT]\n┗┅⭑ ${Aku} Point\n\n🎉 *WIN* 🎉\nKamu mendapatkan ${count * 2} Uang(xp)`.trim()
    conn.sendButton(m.chat, caption, wm, imgr + "WINNER", [["Try Again " + args[0], "/casino " + args[0]]], m, { mentions: conn.parseMention(caption) })
    } else {
