...
let linkq = 'https://mfarels.my.id/api/' + args[0] + '?q=' + one
        ...
        return conn.sendButton(m.chat, args[0], nofeq, null, [['Back', '.?']], m)
        }
        if (!scanq == "application/json") {
        return conn.sendButton(m.chat, args[0], wm, linkq, [['Back', '.?']], m)
        }
            break
            case 'bahasa-g':
case 'bajinganlo':
...
let linkt = 'https://mfarels.my.id/api/' + args[0] + '?text=' + one
        ...
        return conn.sendButton(m.chat, args[0], nofet, null, [['Back', '.?']], m)
        }
        if (!scan == "application/json") {
        return conn.sendButton(m.chat, args[0], wm, linkt, [['Back', '.?']], m)
        }
            break
}
}
